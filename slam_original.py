...
# TODO: edit for our observation model ?
# RFID -> landmarks, u -> move, xd -> primar position with noise, xTrue -> real primar position
def observation(xTrue, xd, u, RFID):
    xTrue = motion_model(xTrue, u) #real position

   
    # noise to landmarks observations
    z = observe_landmarks(xTrue, RFID)
    ...
    xd = motion_model(xd, u_with_noise) # predicted position with noise 
    return xTrue, z, xd, u_with_noise


def observe_landmarks(xTrue, RFID):
    # add noise to gps x-y
    z = np.zeros((0, 3))

    # data assosiation we know which landmark is being observed 
    for i in range(len(RFID[:, 0])):
        dx = RFID[i, 0] - xTrue[0, 0]
        dy = RFID[i, 1] - xTrue[1, 0]
        d = math.hypot(dx, dy)          #real distance
        angle = pi_2_pi(math.atan2(dy, dx) - xTrue[2, 0]) #real angle
        if d <= MAX_RANGE:
...
def get_movement_and_observations():
...
def main():
    ...
    while SIM_TIME >= time:
        ...
        #(real_position, noise_to_observations, position_with_noise_to_move, move_with_noise)
